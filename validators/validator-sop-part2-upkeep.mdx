...
First, go to the Geth Repository [here](https://geth.ethereum.org/downloads/) and right-click on the Geth for Linux button and then click copy link. Be sure to copy the correct link. It should look something like `https://gethstore.blob.core.windows.net/builds/geth-linux-amd64-1.14.0-87246f3c.tar.gz`. Modify the URL in the instructions below to match the download link for the latest version (x5), which is v1.14.0 in this example (current as of 4.24.24).
...
Note: [ONLY] If updating from a version prior to 1.14.0 (where you have yet to switch to the new database schema), you will need to delete your database and rebuild it. To delete the database, use the following command:

```
sudo -u geth geth removedb --datadir=/var/lib/geth
```

When you're asked if you want to delete the "state" and "ancient chain", answer Y to both promopts to start with a fresh database. Geth will automatically rebuild the database when you restart the service below.
...
First, let's check the latest version of Prysm available on their GitHub repo, which can be accessed [here](https://github.com/prysmaticlabs/prysm/releases). Be sure to copy the correct link, it should look something like `https://github.com/prysmaticlabs/prysm/releases/download/v5.0.3/beacon-chain-v5.0.3-darwin-amd64.sha256`. We will need to modify the commands below to match the latest version number, which is v5.0.3 in this example (current as of 4.5.24).
