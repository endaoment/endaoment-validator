...
First, go to the Reth Repository [here](https://github.com/paradigmxyz/reth/releases) and expand the Assets section for the latest release. Copy the download link for the `reth-v...-x86_64-unknown-linux-gnu.tar.gz` file. Be sure to copy the correct link. It should look something like `https://github.com/paradigmxyz/reth/releases/download/v1.10.2/reth-v1.10.2-x86_64-unknown-linux-gnu.tar.gz`. Modify the URL in the instructions below to match the download link for the latest version, which is v1.10.2 in this example (current as of Jan 2026).

```
cd ~
curl -LO https://github.com/paradigmxyz/reth/releases/download/v1.10.2/reth-v1.10.2-x86_64-unknown-linux-gnu.tar.gz
```
...
```
tar xvf reth-v1.10.2-x86_64-unknown-linux-gnu.tar.gz
...
```
cd ~
rm reth-v1.10.2-x86_64-unknown-linux-gnu.tar.gz
reth --version
```
...
```
cd ~
curl -LO https://github.com/flashbots/mev-boost/releases/download/v1.10.2/mev-boost_1.10.1_linux_amd64.tar.gz
```
...
**⚠️ Important:** MEV-Boost v1.10.2 is a required update for the upcoming Fusaka upgrade (epoch 411392, scheduled for Dec-03-2025 09:49:11 PM UTC). This version includes support for the new GetPayloadV2 API and Prometheus metrics.
...
The latest version of MEV-boost can be found [here](https://github.com/flashbots/mev-boost/releases). Simply scroll to the bottom of the linked releases page, and right click the .tar.gz file that contains `linux_amd64`. Copy that link and replace all file names and links below, which uses v1.10.2, latest as of 11.14.25.
