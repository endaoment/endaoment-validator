...
## A Quick Note: Client Diversity

This guide uses a **Reth/Prysm** setup. We’re intentionally running **minority clients** (especially on the execution layer) and we hope this guide is helpful for anyone looking to run Reth and/or Prysm and contribute to client diversity.

For current client distribution, see: [Client diversity](https://clientdiversity.org/)

Client diversity remains important for network security and resilience (e.g., reducing correlated client risk). Our goal with this setup is to contribute to that diversity while maintaining the reliability and security standards required for our nonprofit operations.
