...
```
cd /var/lib/reth
SIZE=$(curl -sI https://downloads.merkle.io/reth-YYYY-MM-DD.tar.lz4 | grep -i content-length | awk '{print $2}' | tr -d '\r')
sudo -u reth wget -q -O - https://downloads.merkle.io/reth-YYYY-MM-DD.tar.lz4 | \
  pv -s $SIZE -pterab | sudo -u reth tar -I lz4 -xf -
```
...
Example for December 15, 2025:
```
cd /var/lib/reth
SIZE=$(curl -sI https://downloads.merkle.io/reth-2025-12-15.tar.lz4 | grep -i content-length | awk '{print $2}' | tr -d '\r')
sudo -u reth wget -q -O - https://downloads.merkle.io/reth-2025-12-15.tar.lz4 | \
  pv -s $SIZE -pterab | sudo -u reth tar -I lz4 -xf -
```
