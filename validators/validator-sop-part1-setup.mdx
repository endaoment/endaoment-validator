...
These are the same ports we allowed above in the _Secure the Server_ step. Port forwarding these will make it easier for the execution client (Reth) to find suitable peers and accept inbound connections. Failure to do so may result in sub optimal syncing and reduced peer discovery.
...
**Note:** Right after installation, chrony needs 5-10 minutes to synchronize with time servers. The "Leap status" will show "Not synchronised" initially - this is normal.
...
* Compressed snapshot: ~2.1TB
* Uncompressed database: ~3TB
* **With a 4TB SSD, you MUST use streaming method** - downloading tar first requires 5TB+ (2.1TB tar + 3TB extracted)
* Streaming only stores the extracted database (~3TB), leaving adequate free space

**IMPORTANT:** Use `tmux` to run the download to prevent it from being killed if your SSH connection drops. The download takes 18-20 hours.
...
```
cd /var/lib/reth
sudo -u reth wget -O - https://downloads.merkle.io/reth-YYYY-MM-DD.tar.lz4 | \
  sudo -u reth tar -I lz4 -xvf -
```
...
Example for October 16, 2025:
```
cd /var/lib/reth
sudo -u reth wget -O - https://downloads.merkle.io/reth-2025-10-16.tar.lz4 | \
  sudo -u reth tar -I lz4 -xvf -
```
...
**If not using a snapshot:** Reth will sync from genesis with the `--full` flag, which takes 1-3 days. Skip the snapshot download and proceed directly to starting the service.
