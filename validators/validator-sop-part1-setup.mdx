...
Time to install Geth! All commands provided below are based on the 1.16.6 version of Geth (current as of 11.3.25), but should be adjusted based on whatever the latest version of Geth is [here](https://geth.ethereum.org/downloads) - simply right click and copy the URL on the 'FOR LINUX' box. Note that MANY commands below will need to update if this version is updated.

**Note:** v1.16.6 enables the Fusaka hardfork on Ethereum mainnet (scheduled for 2025-12-03 21:49:11 UTC) as well as BPO1 and BPO2 forks.

Curl the Geth build from the above link:

```
cd ~
curl -LO https://gethstore.blob.core.windows.net/builds/geth-linux-amd64-1.16.6-386c3de6.tar.gz
```
...
```
tar xvf geth-linux-amd64-1.16.6-386c3de6.tar.gz
cd geth-linux-amd64-1.16.6-386c3de6
...
```
cd ~
rm geth-linux-amd64-1.16.6-386c3de6.tar.gz
rm -r geth-linux-amd64-1.16.6-386c3de6
```
