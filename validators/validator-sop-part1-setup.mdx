...
```
sudo systemctl daemon-reload && sudo systemctl restart ssh && sudo systemctl restart ssh.socket
```
...
_Note: Reth uses port 30303 by default for P2P networking. If you need to customize the networking ports, consult the [Reth documentation](https://github.com/paradigmxyz/reth) for the appropriate flags and ensure those ports are allowed in your firewall._
...
You should see the text "passwd: password changed."
...
This is the same port we allowed above in the _Secure the Server_ step - port forwarding 30303 will make it easier for the execution client (Reth) to find suitable peers. Failure to do so may result in sub optimal syncing.
...
```
tar xvf reth-v1.8.2-x86_64-unknown-linux-gnu.tar.gz
...
```
cd ~
rm reth-v1.8.2-x86_64-unknown-linux-gnu.tar.gz
```
...
After running Reth for ~10 or so minutes, check the peer count to ensure it's greater than 0.
