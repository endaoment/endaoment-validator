...
- [Step 5: Configure Reporting Software](/validators/validator-sop-part4-monitoring#step-5-configure-reporting-software)
    - [Configure JSON Exporter](/validators/validator-sop-part4-monitoring#configure-json_exporter)
...
- [Step 6: Configure Prometheus Jobs + Targets](/validators/validator-sop-part4-monitoring#step-6-configure-prometheus-jobs--targets)
...
Copy and paste text into this configuration file so it matches [prometheus.yml](https://github.com/endaoment/endaoment-validator/blob/main/.imgs/prometheus.yml).
...
Edit the configuration file for json\_exporter:

```
sudo nano /etc/json_exporter/json_exporter.yml
```

Copy and paste the contents of [this file](https://github.com/endaoment/endaoment-validator/blob/main/.imgs/json%5Fexporter.yml) into that file. Then press CTRL + X followed by Y to save.
...
Copy and paste the contents of [this file](https://github.com/endaoment/endaoment-validator/blob/main/.imgs/stakelocal%5Frules.yml) into that file. Then press CTRL + X followed by Y to save.
...
Log into your Grafana instance `({YourIp}:3000)` using the password you set up earlier. In the menu, click on Dashboard > New > Import. Copy and paste [this file](https://github.com/endaoment/endaoment-validator/blob/main/.imgs/grafana-dashboard.json) into the `Import via panel json` area and click `Load`. On the next screen, give your dashboard a unique name, feel free to change the UID, and click `Import`.
