...
```
cd ~
curl -LO https://github.com/prometheus/prometheus/releases/download/v3.7.2/prometheus-3.7.2.linux-amd64.tar.gz
tar xvf prometheus-3.7.2.linux-amd64.tar.gz
sudo cp prometheus-3.7.2.linux-amd64/prometheus /usr/local/bin/
sudo cp prometheus-3.7.2.linux-amd64/promtool /usr/local/bin/
```

Modify file names and clean up:

```
sudo cp -r prometheus-3.7.2.linux-amd64/consoles /etc/prometheus
sudo cp -r prometheus-3.7.2.linux-amd64/console_libraries /etc/prometheus
rm prometheus-3.7.2.linux-amd64.tar.gz
rm -r prometheus-3.7.2.linux-amd64
```
...
These errors are because we need to update the data source for all panels you just imported. Head back to the data sources menu (Home > Connection > Data sources) and click on Prometheus. From here, click on Dashboards, and click `Build a Dashboard` (we will delete this later). Click `Import dashboard` save the dashboard, and then click it again - finally, use the import code `3662`, then click load, select the prometheus data source, then import.
