...
Install go (version 1.23+ required for ethereum-metrics-exporter):
...
Download the latest release of Prometheus by going [here](https://prometheus.io/download/) and copying the download link for the latest linux-amd64.tar.gz file. Please be sure to replace the link below using the current version. Download the repository, extract the files, and move them to the /bin:
...
rm prometheus-3.8.1.linux-amd64.tar.gz
rm -r prometheus-3.8.1.linux-amd64
```
...
If it says "active (running)" in green text, you've done it! Press Q to quit (this will not affect the Prometheus service).
...
**Troubleshooting Note:** If Prometheus fails to start with "address already in use" errors, you may have a snap-installed Prometheus conflicting with the manual installation. Check with `snap services` and if Prometheus is installed via snap, remove it with `sudo snap remove prometheus`.

#### Node Exporter

Download the latest release of Node Exporter by going [here](https://prometheus.io/download/#node%5Fexporter) and copying the download link for the latest linux-amd64.tar.gz file. Please be sure to replace the link below using the current version. Download the repository, extract the files, and move them to the /bin:
...
If it says "active (running)" in green text, you've done it! Press Q to quit (this will not affect the Node Exporter service).
...
**Note:** Some hardware configurations (particularly VMs or systems without thermal sensors) may trigger recurring `thermal_zone` collector errors in the logs. These warnings are harmless and can be safely ignored. If desired, you can disable this collector by adding `--no-collector.thermal_zone` to the `ExecStart` line in the service file.

#### JSON Exporter

Download the latest release of JSON Exporter by going [here](https://github.com/prometheus-community/json%5Fexporter/releases) and copying the download link for the latest linux-amd64.tar.gz file. Please be sure to replace the link below using the current version. Download the repository and move them to /git:

```
mkdir ~/git
cd git
git clone --depth 1 --branch v0.7.0 https://github.com/prometheus-community/json_exporter.git
cd ~
```
...
It may not say "active (running)" in green text, as `'/etc/json_exporter/json_exporter.yml' does not exist`, this is expected. We will configure the YML file in step 5 below.
...
Clone and build the latest version of the Ethereum Metrics Exporter from source:
...
Copy the executable to /usr/local/bin and verify it works:

```bash
sudo cp ethereum-metrics-exporter /usr/local/bin/
sudo chmod +x /usr/local/bin/ethereum-metrics-exporter
/usr/local/bin/ethereum-metrics-exporter --help
```
...
```bash
cd ~
rm -rf ~/git/ethereum-metrics-exporter
```
...
It may not say "active (running)" in green text, as `'/etc/eth-metrics/eth-metrics.yml' does not exist`, this is expected. We will configure the YML file in step 5 below.
...
You may get a warning about apt-key being deprecated, it is ok to ignore this warning.
...
You may need to press `Y` to continue.
...
If it says "active (running)" in green text, you've done it! Press Q to quit (this will not affect the Grafana service)
...
**Important:** Make sure the consensus URL uses `http://` (not `https://`) for local connections.
...
Download the stakelocal configuration files directly from the GitHub repository:

```bash
cd ~
sudo apt install unzip
wget https://github.com/endaoment/endaoment-validator/archive/refs/heads/main.zip
unzip main.zip
```

Copy the stakelocal folder to the correct location:

```bash
sudo mkdir -p /etc/prometheus/files_sd/stakelocal/
sudo cp -r endaoment-validator-main/.imgs/stakelocal/* /etc/prometheus/files_sd/stakelocal/
```

Clean up the downloaded files:

```bash
rm -rf endaoment-validator-main main.zip
```
...
Edit the validator configuration file to include a list of comma separated public keys of your validator(s):
...
Make sure to replace `{validators}` with a comma separated list of your validator public keys or indices (discoverable via Beaconcha.in if unknown). **Important:** Each validator key must be enclosed in quotes, in the style of `-targets: [ "0x123...", "0x456...", ... "0xXYZ..." ]`.
...
Now, check the status of each of these services:
...
Log into your Grafana instance `({YourIp}:3000)` using the password you set up earlier. In the menu, click on Dashboard > New > Import. Copy and paste [this file](../.imgs/grafana-dashboard.json) into the `Import via dashboard JSON model` area and click `Load`. On the next screen, give your dashboard a unique name, feel free to change the UUID, and click `Import`.

⚠️ **You're going to see a lot of errors, that is expected!** ⚠️

These errors are because we need to update the data source for all panels you just imported. Head back to the data sources menu (Home > Connection > Data sources) and click on Prometheus - ensure the connection is set to http://localhost:9090 with `No Authentication`, then scroll down and click `Save and Test`. 

Next, click on Dashboards, and click `New` then `Import` (we will delete this later), use the import code `3662`, then click load, select the prometheus data source, then import.

Navigate to that dashboard (it should automatically open), click edit and head to settings, and then click on JSON Model. Copy the UUID contained in line 19\. This is your data source's UUID!

After ensuring you have the UUID copied, You can head to Dashboards and delete all additional ones we just created. Just make sure you hang onto that UUID.

Armed with the correct UUID, let's fix the validator dashboard. Click on Dashboards at the top of the screen, and navigate back to your dashboard. From there, click edit and head to settings, and then click on JSON Model. Press CTRL + F and click the down arrow to prepare a find and replace. Paste your data sources UUID into the Replace section, and `YOURUUID` into the Find section (note this is the UUID in line 37). Finally, click the replace all button to update your dashboard. To save, FIRST click `save changes` at the bottom and THEN `save dashboard` at the top of the screen, and finally click `save`.
